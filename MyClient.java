    ...
    public static void main(String[] args){
		try {
			...
			*/
			...
			int serverCounter = 0; //Counter for Iterating Array Consisting of Servers
			String str;
			...
			while (true) {
                ...
                send("REDY");
                String job = recieve(); // JOBN Stored to Job.
                if(job.equals("NONE")){
                    break;
                }
				...
				if (!(job.equals("NONE"))) {
					...
					int jobID = Integer.parseInt(job.split(" ")[2]);
					...
					//Boolean Flag to run conditional only once
					boolean sortServers = true;
					if(sortServers){
					...
					String[] serverToSchedule = largestServers.get(serverCounter).split(" ");
					...
					if (str.equals(".") && job.split(" ")[0].equals("JOBN")) {
						...
						if(serverCount == largestServers.size()) {
							serverCount = 0;
						}
					}
				}
